...
**NOTE:** Complete the assignment in your local and send it to us as a zip file
...
5. Run web and API apps:

```bash
pnpm dev
```

The API starts at `http://localhost:3001` and Next.js starts at `http://localhost:3000`.
...
- `ADMIN_API_KEY`: key for protected event creation endpoint
...
- `DATABASE_URL`: PostgreSQL connection string used by Drizzle CLI and seed scripts

## Testing

Run API tests (unit + integration + concurrency):

```bash
pnpm --filter api test
```

Run pricing coverage report:
